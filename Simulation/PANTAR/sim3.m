%% Minimal version simulation
function prosthetic_foot_sim_v5()
    ...
    n = 3;           % Gear ratio (ankle joint)
    p = 0.005;       % Ball screw lead (m/rev)
    
    k3 = 1000;       % Spring s3 stiffness (N/m)
    L3 = 0.12;       % Spring s3 rest length (m)

    n_t = 5;         % Toe joint gear ratio (output shaft speed / motor speed)

    % Toe joint spring neutral angle (rad)
    theta_t0 = 0;
    ...
    % Solve ODE
    [t, y] = ode45(@(t,y) dynamics_v5(t, y, I_a, I_t, k1, L0, n, p, k3, L3, n_t, theta_t0), tspan, y0);
    ...
    visualize_motion_v5(t, y);
end

function dydt = dynamics_v5(t, y, I_a, I_t, k1, L0, n, p, k3, L3, n_t, theta_t0)
    theta_a = y(1);
    omega_a = y(2);
    x_m = y(3);
    theta_t = y(5);
    ...
    % Ankle motor input (angular speed)
    v1 = 2 * sin(2*pi*0.5*t); % Motor angular speed rad/s
    v2 = n * v1;              % Screw rotational speed
    v_m = p * v2;             % Nut velocity

    % Ankle joint calculations
    O1 = [0; -0.06];
    P_m = [x_m; -0.09];
    ...
    % Toe joint spring torque, assume stiffness k_t = 0.2 N·m/rad
    k_t = 0.2;
    torque_spring_t = -k_t * (theta_t - theta_t0);

    % Toe joint total torque = motor torque through gear + spring torque
...
function visualize_motion_v5(t, y)
    for i = 1:10:length(t)
        theta_a = y(i,1);
        x_m = y(i,3);
        theta_t = y(i,5);
        ...
        % Foot plate fixed point near O1, assume connecting rod starts from the foot plate edge
        P0 = [-0.03; -0.12];
        P1 = [0.16; -0.12];
